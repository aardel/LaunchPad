// Default Launchpad server URL
const LAUNCHPAD_SERVER = 'http://localhost:5174';
...
async function checkConnection() {
  ...
  try {
    ...
    const response = await fetch(`${LAUNCHPAD_SERVER}/api/groups`, {
      method: 'GET',
    ...
    if (response.ok) {
      ...
      statusIcon.textContent = '✓';
      statusText.textContent = 'Launchpad is running';
    ...
    if (error.name === 'AbortError') {
      ...
      statusText.textContent = 'Launchpad not running (click to retry)';
    }
    statusDiv.classList.add('clickable');
    return false;
  }
...
async function loadGroups() {
  ...
  try {
    ...
    if (groups.length === 0) {
      groupSelect.innerHTML = '<option value="">No groups available. Create a group in Launchpad first.</option>';
    ...
    groupSelect.innerHTML = '<option value="">Error: Launchpad not running or connection failed</option>';
    groupSelect.disabled = false;
...
document.getElementById('bookmarkForm').addEventListener('submit', async (e) => {
  ...
  try {
    const response = await fetch(`${LAUNCHPAD_SERVER}/api/bookmarks`, {
      method: 'POST',
      headers: {
    ...
    submitText.textContent = 'Add to Launchpad';
